...
3.  How many variables are in the data set?
    ...
    --\>8 variables (species, island, bill_length_mm, bill_depth\_ mm, flipper_length_mm, body_mass_g, sex, year
...
5.  What data types are contained in the variables? (Reminder: [https://rbtl-fs24.github.io/website/slides/lec-02-visualisation.html#/types-of-variables](https://ds4owd-001.github.io/website/slides/lec-02-visualisation.html#/types-of-variables){.uri})
    ...
    --\>discrete variables (countable quantities): year

    --\>countable variables (can take any value within a range): bill_length_mm, bill_depth\_ mm, flipper_length_mm, body_mass_g

6.  --\>categorical variables (distinct groups): species, island, sex
...
```{r}
penguins_noChinstraps <-penguins |>
...
```{r}
penguins_Gentoo_15.5mm <-penguins |>
...
```{r}
penguins_male_DreamorBiscoe <- penguins |> 
...
```{r}
penguins_Adelie_35mm_female <-penguins |>
...
```{r}
penguins_Chinstrap_female_DreamorTorgersen <- penguins |> 
  filter(island == "Dream" | island== "Torgersen", sex=="female", species=="Chinstrap" )
```
...
```{r}
penguins_female_TorgersenorBiscoe <- penguins |> 
  filter(sex == "female" , (island == "Torgersen" | island == "Biscoe")) |> 
  count()
print(penguins_female_TorgersenorBiscoe)
```
