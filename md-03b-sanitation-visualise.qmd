...
3.  How many variables are in the data set?
    ...
    --\>8 variables (name, iso3, year, region_sdg, varname_short, varname_long, residence, percent)
...
```{r}
sanitation_Nigeria_2020or2000_sm <-sanitation |> 
  filter(name == "Nigeria",
         year == 2020 | year== 2000 ,
...
```{r}
count(sanitation_Nigeria_2020or2000_sm)
```
...
```{r}
ggplot(sanitation_Nigeria_2020or2000_sm,
       aes(x = residence,
           y = percent,
           fill = varname_long
           )) +
...
1.  Look at the plot that you created. What do you notice about the order of the bars / order of the legend?
    ...
    --\>The bars are stacked in a specific order, which correspinds to the order of varname_long in the dataset. This order is also reflected in the legend.

2.  What would you want to change?

    --\>it would make more sense to present the data in a logical sequence, like progression from least to the most improved sanitation facilities

3.  Why did we remove "safely managed sanitation services" from the data set in Task 3?

    --\>safely managed sanitation services category has a lot of missing data (a lot of NA) as can be seen below
...
2.  What do you observe when you look at the code and plot?
    ...
    --\>plot shows a series of histograms based on the sanitation_2020 sub dataset (sanitation has been filtered to only include data from year 2020)

    --\>each histogram represents the distribution of the "percent" variable within a particular category of "varname_short"across different residences

    --\>you can draw different conclusions from these plots, e.g. how certain sanitation services are more or less prevalent in rural areas compared to urban ones or how the distribution of percentages for a given service type varies by residence
